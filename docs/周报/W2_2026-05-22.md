...
> **方向校准说明**：本期收到导师反馈（详见 [`docs/决策记录/2026-05-22_导师反馈方向校准.md`](https://github.com/bob798/doc-qna-openvino/blob/main/docs/决策记录/2026-05-22_导师反馈方向校准.md)），W3 收敛到端到端 Demo 主线，评测体系扩张和 NPU 全链路验证降为加分项。下周计划按"主线 / 必做评测瘦身版 / 加分项"三档组织。

**主线（必做，P0）— 端到端 Demo 跑通**

1. **Phase 3.1 · Embedding + ChromaDB**：BGE-small-zh 转 OpenVINO IR（INT8）→ ChromaDB 持久化 → Phase 2 输出的 chunks.jsonl 灌入 → 人工验证 Top-K 召回相关性。
2. **Phase 3.2 · LLM 生成（Qwen3-1.7B INT4）**：用 `openvino_genai.LLMPipeline` 加载 `OpenVINO/Qwen3-1.7B-int4-ov`，RAG prompt 模板设计 + 输出带 (doc_name, page) 引用，`enable_thinking=False` 关思考模式。
3. **Phase 3.3 · 端到端 `scripts/run_qa.py`**：1~2 份 PDF → 解析 → 入库 → 提问 → 答案，跑 **3~5 条业务问题**记录耗时分解（embed / retrieve / LLM）。
4. **README + 复现物料并行启动**：端到端通的当晚回填运行命令、样例 I/O、终端截图、性能数据，不拖到最后一周。
5. **NPU 限制说明（不再硬试全链路）**：当前 IR 含 PP-DocLayoutV3，NPU 暂不支持完整 OCR 链路；README/最终材料用一句话写清"主线 CPU/GPU/AUTO，NPU 作探索性结果"。

**必做评测（瘦身版，P1）**

6. **业务问题集精简**：从 `eval_questions.jsonl` 18 题挑 3~5 题最能说明效果的，作为 Demo 主证据。
7. **Tesseract vs PaddleOCR-VL 少量对比**：选 2~3 个代表性页面（含表格 + 中文 + 公式）人工对比识别质量，不跑完整 OmniDocBench。

**加分项（P2-P3，时间不够直接砍）**

8. **NPU 进取路径**：把 Qwen3 LLM 或 BGE Embedding 切到 NPU 跑（NPU 明确支持的 workload），讲"OCR-GPU / LLM-NPU / 检索-CPU"AI PC 全家桶叙事。
9. **OmniDocBench 子集** Edit Distance / TEDS / CDM、**RAGAS** faithfulness / answer_relevancy、**业务题扩到 30~50**、**Gradio UI**。

---

### 当前阻塞（无则填"无"）

- **NPU 全链路不可行（已明确）**：导师反馈指出 NPU 当前仅支持 PP-DocLayoutV2，本项目所用 IR `zhaohb/PaddleOCR-VL-1.5-ov` 含 PP-DocLayoutV3 算子，NPU 暂无法跑完整 OCR 链路。已决策：主线用 CPU/GPU/AUTO；NPU 改为限制说明 + 加分项探索（把 LLM/Embedding 这类 NPU 明确支持的 workload 切上去，凸显 AI PC 全家桶价值）。**不再投入时间硬试 V3 NPU**。
